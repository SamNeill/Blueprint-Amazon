...
class RequestFragment : BaseFramesFragment<RequestApp>() {
    ...
    private val blueprintPrefs: BlueprintPreferences by lazy { BlueprintPreferences(requireContext()) }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (!blueprintPrefs.iconsRequestConsentAccepted) {
            (activity as? BlueprintActivity)?.showConsentDisclaimer {
    ...
    private fun onCheckChange(app: RequestApp, checked: Boolean) {
        if ((activity as? BlueprintActivity)?.changeRequestAppState(app, checked) == true) {
            ...
            (activity as? BlueprintActivity)?.updateFab(force = true)
        } else {
            requestAppsAdapter.untoggle(app)
        }
    }
