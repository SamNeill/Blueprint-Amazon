...
abstract class BlueprintActivity : FramesActivity(), RequestCallback {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        homeFragment?.showDonation(true)
        ...
        try {
            val jsonUrl = resources.getString(dev.jahir.frames.R.string.json_url)
            if (jsonUrl.isNullOrEmpty()) {
                bottomNavigation?.menu?.findItem(R.id.wallpapers)?.isVisible = false
            }
        } catch (e: Exception) {
            bottomNavigation?.menu?.findItem(R.id.wallpapers)?.isVisible = false
        }

        android.util.Log.d("Blueprint", "Donation Items: ${getDonationItemsIds()}")
        android.util.Log.d("Blueprint", "Billing Ready: $isBillingClientReady")
    }
    ...
    private fun buildRequest() {
        ...
        // First check if any apps are selected
        if (requestsViewModel?.selectedApps.isNullOrEmpty()) {
            showRequestDialog {
        ...
        if (!blueprintPrefs.iconsRequestConsentAccepted) {
            ...
            // If consent already accepted, directly send request
            SendIconRequest.sendIconRequest(this, requestsViewModel?.selectedApps, this)
        }
    }
    ...
    override fun onBillingClientReady() {
        ...
        android.util.Log.d("Blueprint", "Billing Client Ready")
        val donationItems = getDonationItemsArray()
        val hasDonationItems = donationItems.isNotEmpty() && donationItems[0].isNotEmpty()
        homeFragment?.showDonation(hasDonationItems && isBillingClientReady)
    }
    ...
    internal fun showConsentDisclaimer(onAccept: () -> Unit) {
        dismissRequestDialog()
        requestDialog = mdDialog {
            title(string(R.string.consent_requirement))
            message(string(R.string.consent_requirement_content))
            cancelable(false)
            positiveButton(string(R.string.accept)) {
                it.dismiss()
                onAccept()
            }
            neutralButton(string(R.string.deny)) {
                it.dismiss()
    ...
    open fun getDonationItemsArray(): Array<String> {
        return try {
            // Try to get the resource ID from the app package
            val resId = resources.getIdentifier("donation_items", "array", packageName)
